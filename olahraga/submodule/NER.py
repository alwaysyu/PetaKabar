...
class NER:
    def __init__(self) -> None:
        ...
        self.listDaerah = joblib.load('D:/Repository/PetaKabar/olahraga/model/Where/listProvKabKec.pkl')
        self.listProvinsi = joblib.load('D:/Repository/PetaKabar/olahraga/model/Where/provinsi.pkl')
        self.listKabupaten = joblib.load('D:/Repository/PetaKabar/olahraga/model/Where/kabupaten.pkl')
        self.listKecamatan = joblib.load('D:/Repository/PetaKabar/olahraga/model/Where/kecamatan.pkl')
        self.loaded_model = load('D:/Repository/PetaKabar/olahraga/model/NER_Kec_Ben_Krim_Kese_Eko/output/model-last')

        self.newsscrapped = []
        try:
            cnx = mysql.connector.connect(user='admin', password='admin', database = 'Petakabar')
            cursor = cnx.cursor()
            cursor.execute("SELECT ID, berita_date, berita_desc FROM berita where berita_topik_id = 6 AND class_classification is null")
