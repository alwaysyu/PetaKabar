...
class Severity:
    def __init__(self) -> None:
        ...
        self.ct.set_model_file('D:/Repository/PetaKabar/bencana/main/model/TagIndo/all_indo_man_tag_corpus_model.crf.tagger')
    ...
    def getKeparahanVelue(self):
        try:
            ...
            for i in range(0, len(self.newsscrapped)):
                ...
                self.save_to_mysql(idberita, death, injury, house, person)

            #     document_result.append([title, berita, time, what, tanggal_asli, orang, provinsi, kabupaten, kecamatan, death, injury, house, person])

            # writer = pd.DataFrame(document_result, columns=[
            #                     'title', 'description', 'time', 'what', 'when', 'who', 'provinsi', 'kabupaten / kota', 'kecamatan', "mati", 'luka', 'rumah', 'orang'], index=None)
            # writer.to_csv('result/tagging/severity_count.csv', index=False, sep=',')
            return "success"
        except:
            return "error"
    
    def save_to_mysql(self, idberita, death, injury, house, person):
        try:
            conn = mysql.connector.connect(user='admin', password='admin', database = 'Petakabar')
            cur = conn.cursor()
            add_news = ("UPDATE berita "
                        "SET sev_death = %s, sev_injury = %s, sev_house = %s, sev_person = %s "
                    "WHERE ID = %s"
                    )
            data_news = (death, injury, house, person, idberita)
