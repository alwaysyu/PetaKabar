...
class NER:
    def __init__(self) -> None:
        ...
        self.listDaerah = joblib.load('D:/Repository/PetaKabar/ekonomi/model/Where/listProvKabKec.pkl')
        self.listProvinsi = joblib.load('D:/Repository/PetaKabar/ekonomi/model/Where/provinsi.pkl')
        self.listKabupaten = joblib.load('D:/Repository/PetaKabar/ekonomi/model/Where/kabupaten.pkl')
        self.listKecamatan = joblib.load('D:/Repository/PetaKabar/ekonomi/model/Where/kecamatan.pkl')
        self.listNegara = joblib.load('D:/Repository/PetaKabar/ekonomi/model/Where/list_negara.pkl')
        self.loaded_model = load('D:/Repository/PetaKabar/ekonomi/model/NER_Kec_Ben_Krim_Kese_Eko/output/model-last')
    ...
    def getLocationByCountry(self,negara):
        if( negara=='indonesia'):
    ...
    def getValueNER(self):
        try:
            ...
            for i in range(0, len(self.newsscrapped)):
                        ...
                        if(ent.label_== 'ORANG'):
                            ...
                            if(ent.text[-1:]=='.'):
                                    ...
                                    if ((teks in self.listProvinsi) and (teks not in visitedLokasi)):
                                        ...
                                        visitedLokasi.append(teks)
                                    elif ((teks in self.listNegara) and (teks not in visitedLokasi)):
                                        negara.append([teks,ent.start_char,ent.end_char])
                                        visitedLokasi.append(teks)   
                            else:
                        ...
                        if (self.can_convert_to_int(splitted[0]) and self.can_convert_to_int(splitted[1])):
                            ...
                            if (bulan <= 12):
                                if (bulan == 2 and tanggal <= 29): #handle bulan februari
                                    ...
                                    if (bulan in bulan2 and tanggal < 31): #handle bulan genap
                                        cekdate = True
                ...
                if(lokasi_asli[2]=='-' and lokasi_asli[1]=='-' and lokasi_asli[0]=='-' and negara!='-'):
                        lokasiNegara = self.getLocationByCountry(negara)
                elif((lokasi_asli[2]!='-' or lokasi_asli[1]!='-' or lokasi_asli[0]!='-') and negara=='-'):
                        lokasiNegara = [lokasi_asli[0],lokasi_asli[1],lokasi_asli[2],'-']
                elif((lokasi_asli[2]!='-' or lokasi_asli[1]!='-' or lokasi_asli[0]!='-') and negara!='-'):
                        lokasiNegara = [lokasi_asli[0],lokasi_asli[1],lokasi_asli[2],negara]
                elif(lokasi_asli[2]=='-' and lokasi_asli[1]=='-' and lokasi_asli[0]=='-' and negara=='-'):
                        lokasiNegara = ['-','-','-','-']
                    
                if(len(orang)==0):
                    orang="-"
                self.save_to_mysql(idberita, tanggal_asli, orang, lokasiNegara[2], lokasiNegara[1], lokasiNegara[0], lokasiNegara[3])
    ...
    def save_to_mysql(self, idberita, when, who, provinsi, kabupaten, kecamatan, negara):
        try:
            ...
            for y in range(len(who)):
                ...
                if y < len(who)-1:
                    whostr = whostr + ", "
            data_news = (when, whostr, provinsi, kabupaten, kecamatan, negara, idberita)
