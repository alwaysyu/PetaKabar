...
if __name__ == '__main__':  
    ...
    tag_pos = SequenceTagger.load("D:/Petakabar/models/best-model.pt")
    ...
    for i in range((batch_iter-1)*batch_size, batch_iter*batch_size):
        if len(small_batch) == batch_size:
            ...
            for j in range(len(small_batch)):
                # if j == 1136:
                #       tagged.append(joblib.load('D:/PetaKabar/whowherewhen/3_1136_tagged.pkl'))
                tagged.append(small_batch[j])
            joblib.dump(tagged, 'D:/PetaKabar/whowherewhen/32_all_tagged_'+ iter_through[x-1] + '_' + str(batch_iter) + '.pkl')
            small_batch = []
            tagged = []
            print("*"*30)
            print(batch_iter)
