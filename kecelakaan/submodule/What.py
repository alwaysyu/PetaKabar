...
class What:
    def __init__(self) -> None:
        ...
        self.lda_model = models.ldamodel.LdaModel.load(datapath('D:/Repository/PetaKabar/models/lda_model_kecelakaan/lda_model')) # Ambil Model LDA sesuai dengan topik
        ...
        try:
            ...
            cursor = cnx.cursor()
            cursor.execute("SELECT ID, berita_desc, berita_title FROM berita where berita_topik_id = 3  AND class_classification is null") # adjust
            # cursor.execute("SELECT ID, berita_desc, berita_title FROM berita where berita_topik_id = 3 ") # adjust
