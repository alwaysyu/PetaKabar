...
class NER:
    ...
    def getValueNER(self):
        try:
            ...
            # for x in range(0, len(self.newsscrapped)):
            #     locale.setlocale(locale.LC_TIME, 'id_ID.UTF-8')
            #     qdate = self.dateberita[x]
            #     rep_date = qdate.replace(" WIB","")
            #     date_format = datetime.strptime(rep_date, "%A, %d %b %Y %H:%M")
            #     self.qdateberita.append(date_format)


            for i in range(0, len(self.newsscrapped)):
                # title = self.titleberita[i]
                berita = self.descberita[i]
                time = self.dateberita[i]
                # dateq = self.qdateberita[i]
                # what = self.whatberita[i]
                # listwhat = what.split(", ")
                ...
                for ent in doc.ents:
                        if(ent.label_== 'ORANG'):
                            ...
                            if(ent.text[-1:]=='.'):
                                    ...
                                    if '111' in x:
                                        x = x.replace('111','11')
                                    if (x not in visitedTanggal and self.checkDateFormat(x) != ''):
                                        tanggal.append(x)
                                        visitedTanggal.append(x)
