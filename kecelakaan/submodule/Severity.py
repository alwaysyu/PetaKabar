    ...
    def getKeparahanVelue(self):
        try:
            ...
            for i in range(0, len(self.newsscrapped)):
                ...
                self.save_to_mysql(idberita, death, injury, lost, sink, plane)
            return "success"
        except:
            return "error"

    def save_to_mysql(self, idberita, death, injury, lost, sink, plane):
        try:
            conn = mysql.connector.connect(user='admin', password='admin', database = 'Petakabar')
            cur = conn.cursor()
            add_news = ("UPDATE berita "
                        "SET sev_death = %s, sev_injury = %s, sev_lost = %s, sev_sink = %s, sev_plane = %s "
                    "WHERE ID = %s"
                    )
            data_news = (death, injury, lost, sink, plane, idberita)
